...
gpt2_config={}
#The finetuned DialoGPT models published by Microsoft on azure blob do not have a model config attached to them.
#Model config with vocab and merges is available in /configs folder at https://github.com/microsoft/DialoGPT.git for all the three models.
...
#gpt2_config['medium']=GPT2Config.from_json_file('DialoGPT/configs/345M/config.json')
#gpt2_config['large']=GPT2Config.from_json_file('DialoGPT/configs/762M/config.json')


# Alternatively the model config can also be manually set. These are the default model config for gpt-2 small, medium and large models.
gpt2_config['small'] = GPT2Config()
gpt2_config['medium'] = GPT2Config(n_ctx=1024, n_embd=1024, n_layer=24, n_head=16)
gpt2_config['large'] = GPT2Config(n_ctx=1024, n_embd=1280, n_layer=36, n_head=20)


# load the gpt2 tokenizer. All three gpt2 models (small, medium, large) use the same vocabulary.
# A tokenizer is constructed from two files vocab.json and merges.txt, both of these files are available in configs/117M/,  configs/345M/ and /configs/762M folders
#tokenizer = GPT2Tokenizer.from_pretrained('DialoGPT/configs/345M')

#Alternatively the following line of code will automatically download the vocab.json and merges.txt files and create a tokenizer from these files
tokenizer = GPT2Tokenizer.from_pretrained("gpt2")


#create transformer model from the finetuned model weights
